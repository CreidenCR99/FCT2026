 ...
 */
if ($modo === "paises") {
    $sql = "SELECT DISTINCT pa.Nombre AS Pais FROM Maquinas m LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Paises pa ON pa.Codigo = p.Pais WHERE pa.Nombre IS NOT NULL ORDER BY pa.Nombre";
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) { log_sql_error("Error paises", $sql); http_response_code(500); echo json_encode(["error" => sqlsrv_errors()]); exit(); }
    $data = [];
...
if ($modo === "organismos") {
    $pais = trim($_GET["pais"] ?? "");
    $sql = "SELECT DISTINCT o.Nombre AS Organismo FROM Maquinas m LEFT JOIN Organismos o ON o.Codigo = m.Organismo LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Paises pa ON pa.Codigo = p.Pais WHERE o.Nombre IS NOT NULL AND (? = '' OR pa.Nombre = ?) ORDER BY o.Nombre";
    $params = [$pais, $pais];
    $stmt = sqlsrv_query($conn, $sql, $params);
    if ($stmt === false) { log_sql_error("Error organismos", $sql, $params); http_response_code(500); echo json_encode(["error" => sqlsrv_errors()]); exit(); }
    $data = [];
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) $data[] = $row;
    sqlsrv_free_stmt($stmt);
    responder_json_si_cambia($data, $conn);
...
if ($modo === "provincias") {
    ...
    $pais = trim($_GET["pais"] ?? "");
    $sql = "SELECT DISTINCT p.Nombre AS Provincia FROM Maquinas m LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Organismos o ON o.Codigo = m.Organismo LEFT JOIN Paises pa ON pa.Codigo = p.Pais WHERE p.Nombre IS NOT NULL AND (? = '' OR o.Nombre = ?) AND (? = '' OR pa.Nombre = ?) ORDER BY p.Nombre";
...
if ($modo === "maquinas") {
    ...
    $sqlMachines = "SELECT o.Nombre AS Organismo, p.Nombre AS Provincia, ISNULL(c.Nombre, 'error') AS Cliente, m.Descripcion, m.UltimoControl, m.MonitorizarEstado, m.MonitorizarAlertas, m.NumeroSerie, m.TipoMaquina FROM Maquinas m LEFT JOIN Organismos o ON o.Codigo = m.Organismo LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Paises pa ON pa.Codigo = p.Pais LEFT JOIN Clientes c ON c.Codigo = m.Cliente WHERE (? = '' OR pa.Nombre = ? OR (pa.Nombre IS NULL AND ? = '')) AND (? = '' OR o.Nombre = ? OR (o.Nombre IS NULL AND ? = '')) AND (? = '' OR p.Nombre = ? OR (p.Nombre IS NULL AND ? = '')) ORDER BY o.Nombre, p.Nombre, c.Nombre, m.Descripcion";
    ...
    if (!empty($numeroSeries)) {
        $sqlLogs = "SELECT le.Id as ID, le.NumeroSerie as Numero_Serie, le.TipoMaquina, le.TimeStamp, le.CodigoError, le.Activo, le.Observaciones, e.Descripcion as Mensaje FROM Log_Errores le INNER JOIN Maquinas m ON m.NumeroSerie = le.NumeroSerie LEFT JOIN Organismos o ON o.Codigo = m.Organismo LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Paises pa ON pa.Codigo = p.Pais LEFT JOIN Errores e ON le.CodigoError = e.Codigo WHERE le.Activo = 1 AND (? = '' OR pa.Nombre = ? OR (pa.Nombre IS NULL AND ? = '')) AND (? = '' OR o.Nombre = ? OR (o.Nombre IS NULL AND ? = '')) AND (? = '' OR p.Nombre = ? OR (p.Nombre IS NULL AND ? = '')) ORDER BY le.NumeroSerie, le.Id DESC";
...
if ($modo === "maquinas_navegacion") {
    $sql = "SELECT m.NumeroSerie AS Codigo, m.Descripcion, m.TipoMaquina, m.Notas, m.Organismo, m.Cliente, m.Provincia, m.MonitorizarEstado, m.MonitorizarAlertas, m.UltimoControl, m.Actualizar FROM Maquinas m LEFT JOIN Organismos o ON o.Codigo = m.Organismo LEFT JOIN Provincias p ON p.Codigo = m.Provincia LEFT JOIN Clientes c ON c.Codigo = m.Cliente ORDER BY o.Nombre, p.Nombre, c.Nombre, m.Descripcion";
    $stmt = sqlsrv_query($conn, $sql);
