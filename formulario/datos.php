...
if ($modo === "maquinas_navegacion") {
    $sql = "SELECT m.NumeroSerie, m.Descripcion, m.TipoMaquina, m.Notas, m.Organismo, m.Cliente, m.Provincia, 
                   m.MonitorizarEstado, m.MonitorizarAlertas, m.Actualizar 
            FROM Maquinas m
            LEFT JOIN Organismos o ON o.codigo = m.organismo
...
if ($modo === "maestro") {
    ...
    if ($tipo === "organismos") {
        ...
        $tabla = "Clientes";
    } elseif ($tipo === "maquinas") {
        $sql = "SELECT m.NumeroSerie AS Codigo, m.Descripcion AS Nombre 
                FROM Maquinas m
                LEFT JOIN Organismos o ON o.codigo = m.organismo
                LEFT JOIN Provincias p ON p.codigo = m.provincia
                LEFT JOIN Clientes c ON c.codigo = m.cliente
    ...
    if (!isset($sql)) {
        $sql = "SELECT codigo AS Codigo, Nombre FROM $tabla ORDER BY Nombre";
    }
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) {
        http_response_code(500);
...
if ($modo === "maquinas") {
    ...
    $paramsMachines = [$organismo, $organismo, $provincia, $provincia];
...
if ($modo === "crear_maquina") {
    ...
    $params = [
        // Para el IF EXISTS
        $ns,
        // Para el UPDATE (SET + WHERE)
        $desc, $tipo, $notas, $org, $cli, $prov, $actualizar, $monEstado, $monAlerta, 
        $ns, 
        // Para el INSERT (VALUES)
        $ns, $desc, $tipo, $notas, $org, $cli, $prov, $actualizar, 
        $ultimoControl, 
        $monEstado, $monAlerta
    ];
...
if ($modo === "eliminar_maquina") {
    ...
    if (empty($ns)) {
        http_response_code(400);
        echo json_encode([
            "error" => "No se ha proporcionado el número de serie",
        ]);
        exit();
    }
...
if ($modo === "crear_log") {
    ...
    if (empty($ns) || empty($codigo_error) || empty($fecha) || empty($hora)) {
        ...
        echo json_encode([
            "error" =>
                "Faltan campos obligatorios (Numero Serie, Codigo Error, Fecha, Hora)",
        ]);
        exit();
    }
    ...
    $timestamp =
        str_replace("-", "", $fecha) . str_replace(":", "", $hora) . "00";
