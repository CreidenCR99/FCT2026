...
export function initMachineMaster() {
    ...
    document.getElementById("mmNS").addEventListener("blur", async (e) => {
        ...
        } catch (err) { console.error("Error validando NS:", err); }
    });
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            const res = await fetch("datos.php?modo=crear_maquina", { method: "POST", body: formData });
...
function setupPadding(id, length, type) {
    ...
    const handler = async () => { 
        if (el.value) {
            ...
            } catch (err) { console.error("Error recuperando nombre:", err); }
        } else {
    ...
    el.onblur = handler;
    el.onkeydown = (e) => { if (e.key === "Enter") handler(); };
}
